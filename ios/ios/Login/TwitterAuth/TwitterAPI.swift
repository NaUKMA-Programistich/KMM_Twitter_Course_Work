...
enum TwitterLoginResult: Equatable {
    case none
    case loading
    case error
    case final(TwitterAuthData?)
}

class TwitterAPI: NSObject, ObservableObject {
    @Published var authorizationURL: URL?
    @Published var authState: TwitterLoginResult = .none
    ...
    struct ClientCredentials {
        static let constants = TwitterConstants.shared
        
        static let APIKey = constants.consumerApiKey
        static let APIKeySecret = constants.consumerApiSecretKey
        static let CallbackURLScheme = constants.callback
    }
                        ...
                        if let parameters = url.query?.urlQueryItems {
                            ...
                            self.subscriptions["oAuthAccessTokenSubscriber"] =
                                ...
                                .sink(receiveCompletion: { _ in
                                    ...
                                    self.user = user
                                    self.authState = .final(self.toTwitterAuthData())
                                })
                        }
                    })
            })
    ...
    func toTwitterAuthData() ->  TwitterAuthData? {
        guard let twitterCredentials = self.tokenCredentials else { return nil }
        
        let acessToken = twitterCredentials.accessToken
        let acessSecretToken = twitterCredentials.accessTokenSecret
        let id = self.user?.ID ?? String(acessToken.split(separator: "-")[0]);
        
        return TwitterAuthData(accessToken: acessToken, accessTokenSecret: acessSecretToken, userId: id)
    }
}
